...
func (d *Driver) create(id, parent string, opts *graphdriver.CreateOpts, readOnly bool) (retErr error) {
	...
	if parent != "" {
		...
		xSt, err := idtools.GetOverrideXattr(parentDiff, idtools.ContainersOverrideXattr)
		if err == nil {
			st = xSt
		} else {
	...
	if d.options.forceMask != nil {
		if err := idtools.SetOverrideXattr(diff, idtools.FuseoverlayfsOverrideXattr, st); err != nil {
			return err
		}
	}
